...
describe('@fibbojs/util/FMathUtil should', () => {
  ...
  it('convert quaternion to radian', () => {
    expect(FMathUtil.quaternionToRad({ x: 0, y: 0, z: 0, w: 1 })).toEqual({ x: 0, y: 0, z: 0 })
    const euler1 = FMathUtil.quaternionToRad({ x: Math.sqrt(2) / 2, y: 0, z: 0, w: Math.sqrt(2) / 2 })
    ...
    const euler2 = FMathUtil.quaternionToRad({ x: 0, y: Math.sqrt(2) / 2, z: 0, w: Math.sqrt(2) / 2 })
    ...
    const euler3 = FMathUtil.quaternionToRad({ x: 0, y: 0, z: Math.sqrt(2) / 2, w: Math.sqrt(2) / 2 })
