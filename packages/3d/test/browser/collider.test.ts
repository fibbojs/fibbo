...
function validateRapierCollider(
...
describe("fCollider", () => {
	...
	it("should create a collider with default options", () => {
		...
		expect(cuboid.collider.transform.scaleX).toEqual(
			cuboid.collider.transform.scale.x,
		);
		expect(cuboid.collider.transform.scaleY).toEqual(
			cuboid.collider.transform.scale.y,
		);
		expect(cuboid.collider.transform.scaleZ).toEqual(
			cuboid.collider.transform.scale.z,
		);
		// Validate RAPIER collider
	...
	it("should create a collider with custom transforms", () => {
		...
		expect(cuboid.collider.offset.rotation).toEqual({
			x: -0.3,
			y: 0.2,
			z: 0.1,
		});
	...
	it("should create a collider with a sphere shape", () => {
		...
		expect(cuboid.collider.__COLLIDER__.shape.type).toEqual(
