...
import type {
	FSceneOptions as FSceneOptionsCore,
	FVector3,
} from "@fibbojs/core";
...
export class FScene extends FSceneCore {
	...
	init() {
		...
		this.__PIPELINE_MANAGER__.addStandardPipeline(
			new RenderPipeline({ scene: this }),
		);
	...
	async initPhysics() {
		...
		this.__PIPELINE_MANAGER__.addIntervalPipeline(
			new PhysicPipeline({
				scene: this,
				frameRate: this.__PHYSIC_FRAME_RATE__,
			}),
		);
	}
	...
	handleCollision(
		...
		if (component1 === undefined && component2 === undefined) return;
	...
	addComponent(component: FComponent) {
		...
		// Add mesh to scene
		if (component.__MESH__) this.scene.add(component.__MESH__);
	...
	removeComponent(component: FComponent): void {
		...
		if (component.__MESH__) this.scene.remove(component.__MESH__);

		// Remove colliders and rigidBodies from the scene
		if (component.rigidBody) this.removeRigidBody(component.rigidBody);
		if (component.collider) this.removeCollider(component.collider);
		if (component.sensor) this.removeRigidBody(component.sensor);
