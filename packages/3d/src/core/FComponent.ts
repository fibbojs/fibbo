...
export abstract class FComponent extends FComponentCore {
		...
		if (initiator) {
			...
			if (this.rigidBody) {
				...
				this.__SET_ROTATION__({
					x:
						this.rigidBody.transform.rotationX -
						this.rigidBody.offset.rotationX,
					y:
						this.rigidBody.transform.rotationY -
						this.rigidBody.offset.rotationY,
					z:
						this.rigidBody.transform.rotationZ -
						this.rigidBody.offset.rotationZ,
				});
			}
			// If a collider exists, the propagation comes from the collider
			else if (this.collider) {
	...
	__SET_SCALE__(scale: FVector3): void {
		...
		if (this.__MESH__) {
			...
			if (
				this.__MESH__ instanceof THREE.Mesh &&
				(this.__MESH__.geometry instanceof THREE.BoxGeometry ||
					this.__MESH__.geometry instanceof THREE.SphereGeometry)
			) {
