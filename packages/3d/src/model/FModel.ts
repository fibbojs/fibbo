...
export abstract class FModel extends FComponent {
	...
	constructor(options: FModelOptions) {
		...
		if (!options.path || !options.textures || !options.fileExtension)
			throw new Error(
				"FibboError: path, texture, and fileExtension are required options to create a FModel",
			);
	...
	loadTextureForMesh(mesh: THREE.Mesh) {
		...
		if (!mesh.material || !(mesh.material instanceof THREE.Material)) {
			throw new Error(
				"FibboError: Mesh must have a material to load a texture",
			);
		}
		...
		if (mesh.material.name) {
			...
			if (this.textures[mesh.material.name]) {
				...
				if (this.textures.default) texturePath = this.textures.default;
			...
			textureLoader.load(
				...
				(error) => {
					console.error(
						`FibboError: An error happened while loading the texture for model: name(${this.name}) path(${this.path}) materialName(${mesh.material instanceof THREE.Mesh ? mesh.material.name : "ERROR-NO-MATERIAL-NAME"}) texturePath(${texturePath})`,
					);
					console.error(error);
				},
			);
