...
export class FCharacterControllerD extends FCharacterController {
  constructor(scene: FScene, options: FCharacterControllerOptions) {
    ...
    fKeyboard.on(' ', () => {
      this.component.rigidBody?.rigidBody.applyImpulse(new THREE.Vector3(0, 1, 0), true)
    })
  ...
  onFrame(_delta: number): void {
    ...
    if (this.inputs.forward) {
      this.component.rigidBody?.rigidBody.applyImpulse(cameraDirection.multiplyScalar(0.4 * this.speed), true)
    }
    ...
    if (this.inputs.left) {
      this.component.rigidBody?.rigidBody.applyImpulse(cameraDirection.multiplyScalar(0.4 * this.speed).applyAxisAngle(new THREE.Vector3(0, 1, 0), Math.PI / 2), true)
    }
