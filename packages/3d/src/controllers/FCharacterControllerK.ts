...
export abstract class FCharacterControllerK extends FCharacterController {
	...
	constructor(options: FCharacterControllerOptions) {
		...
		this.characterController =
			this.scene.world.createCharacterController(offset);
		// Configure autostep
	...
	getCorrectedMovements(delta: number): FVector3 {
		...
		movementDirection.z = this.inputs.forward
			? 1
		...
		movementDirection.applyAxisAngle(
		...
		this.characterController.computeColliderMovement(
			this.component.rigidBody
				? this.component.rigidBody.collider.__COLLIDER__
