...
export class FAttachedCamera extends FCamera {
	...
	frame(_delta: number): void {
		...
		this.transform.x =
			this.attachedComponent.__MESH__.position.x + this.offset.x;
		this.transform.y =
			this.attachedComponent.__MESH__.position.y + this.offset.y;
		this.transform.z =
			this.attachedComponent.__MESH__.position.z + this.offset.z;
