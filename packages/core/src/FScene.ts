...
export abstract class FScene {
	 ...
	 */
	public __CALLBACKS_ON_COMPONENT_REMOVED__: ((
	...
	constructor(options?: FSceneOptions) {
		...
		if (typeof window === "undefined" || typeof document === "undefined")
			throw new Error(
				"FibboError: FScene must be instantiated in a browser environment",
			);
		...
		if (
			options.domNode === undefined ||
			options.gravity === undefined ||
			options.physicFrameRate === undefined
		)
			throw new Error(
				"FibboError: The gravity option, DOM node and physic framerate must be defined",
			);
		...
		this.__PIPELINE_MANAGER__ = new PipelineManager({
			scene: this,
			autoLoop: options.autoLoop,
		});
		// Add the main pipeline
		this.__PIPELINE_MANAGER__.addThrottledPipeline(
			new MainPipeline({ scene: this, frameRate: options.mainFrameRate }),
		);
	}
	...
	addComponent(component: FComponent): void {
		...
		this.__CALLBACKS_ON_COMPONENT_ADDED__.forEach((callback) =>
			callback(component),
		);
	}
	...
	removeComponent(component: FComponent): void {
		...
		this.__CALLBACKS_ON_COMPONENT_REMOVED__.forEach((callback) =>
			callback(component),
		);
	}
