...
export abstract class FComponent {
	 ...
	 */
	public __CALLBACKS_ON_COLLISION__: {
		[key: string]: ((data: OnCollisionWithData) => void)[];
	} = {};
	...
	constructor(options?: FComponentOptions) {
		...
		if (options.scene === undefined || options.addToScene === undefined)
			throw new Error(
				"FibboError: FComponent requires addToScene and scene option",
			);
	...
	onCollisionWith(
		...
		return () => {
			this.__CALLBACKS_ON_COLLISION__[eventKey] =
				this.__CALLBACKS_ON_COLLISION__[eventKey].filter(
					(cb) => cb !== callback,
				);
		};
	}
	 ...
	 */
	// biome-ignore lint/suspicious/noExplicitAny: any is required for flexibility
	emitCollisionWith(options: { class?: any; component?: FComponent }) {
