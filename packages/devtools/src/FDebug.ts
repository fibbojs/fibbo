	...
	constructor(scene: FScene) {
		...
		const URL = import.meta.url.includes("/packages/devtools/dist/index.es.js")
			? import.meta.url.replace(
					"/packages/devtools/dist/index.es.js",
					"/packages/devtools/dist/style.css",
				)
		...
		fetch(URL)
			...
			.then((css) => {
				...
				css = css.split('const __vite__css = "')[1].split('\\n"')[0];
		...
		this.scene.components = reactive(this.scene.components) as FComponent[];
