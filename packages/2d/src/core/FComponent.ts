...
import type {
	FComponentOptions as FComponentOptionsCore,
	FVector2,
	OnCollisionWithData,
} from "@fibbojs/core";
...
export abstract class FComponent extends FComponentCore {
	...
	constructor(options?: FComponentOptions) {
		...
		if (
			!options.position ||
			!options.scale ||
			(options.rotation === undefined && options.rotationDegree === undefined)
		)
	...
	render(delta: number): void {
		...
		if (
			...
			const distance = Math.sqrt(
				(this.__CONTAINER__.position.x / 100 - this.transform.__POSITION__.x) **
					2 +
					(-this.__CONTAINER__.position.y / 100 -
						this.transform.__POSITION__.y) **
						2,
			);
			...
			if (distance < 4 && distance > 0.001) {
				...
				// Move the container
				this.__CONTAINER__.position.set(
					newContainerPosition.x * 100,
					-newContainerPosition.y * 100,
				);
			}
	...
	__UPDATE_POSITION__(initiator: boolean = false): void {
		...
		if (initiator) {
			...
			if (this.rigidBody) this.rigidBody.__UPDATE_POSITION__();
			else if (this.collider) this.collider.__UPDATE_POSITION__();
			if (this.sensor) this.sensor.__UPDATE_POSITION__();
		} else {
			...
			if (this.rigidBody) {
				...
				this.transform.__POSITION__ = {
					x:
						...
						this.rigidBody.offset.position.x,
					y:
						this.rigidBody.transform.position.y -
						this.rigidBody.offset.position.y,
				};
			}
	...
	__UPDATE_ROTATION__(initiator: boolean = false): void {
		...
		if (initiator) {
			...
			if (this.rigidBody) this.rigidBody.__UPDATE_ROTATION__();
			else if (this.collider) this.collider.__UPDATE_ROTATION__();
			if (this.sensor) this.sensor.__UPDATE_ROTATION__();
		} else {
			// The event was propagated to the component
			// If a rigidBody exists, the propagation comes from the rigidBody
			if (this.rigidBody) {
				...
				this.__SET_ROTATION__(
					this.rigidBody.transform.rotation - this.rigidBody.offset.rotation,
				);
			}
			// If a collider exists, the propagation comes from the collider
			else if (this.collider) {
				// Rotate the component
				this.__SET_ROTATION__(
					this.collider.transform.rotation - this.collider.offset.rotation,
				);
			}
		}
	}
	...
	__UPDATE_SCALE__(initiator: boolean = false): void {
		...
		if (initiator) {
			...
			if (this.rigidBody) this.rigidBody.__UPDATE_SCALE__();
			else if (this.collider) this.collider.__UPDATE_SCALE__();
			if (this.sensor) this.sensor.__UPDATE_SCALE__();
		} else {
			// The event was propagated to the component
			// If a rigidBody exists, the propagation comes from the rigidBody
			if (this.rigidBody) {
	...
	onCollisionWith(
		...
		if (
			this.sensor &&
			this.sensor.collider.__COLLIDER__.activeEvents() ===
				RAPIER.ActiveEvents.NONE
		) {
			// Set the active events
			this.sensor.collider.__COLLIDER__.setActiveEvents(
