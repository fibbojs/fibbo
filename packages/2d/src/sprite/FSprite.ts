...
export class FSprite extends FComponent {
	...
	constructor(options: FSpriteOptions) {
		...
		this.__CONTAINER__ = new PIXI.Graphics()
			...
			.fill(
				new PIXI.FillGradient(0, 0, 100, 100)
					.addColorStop(0, 0xff00ff)
					.addColorStop(1, 0xffff00),
			) as unknown as PIXI.Sprite;
		// Set the pivot of the container to the center
		this.__CONTAINER__.pivot.set(
			this.__CONTAINER__.width / 2,
			this.__CONTAINER__.height / 2,
		);
	...
	setScaleWidth(width: number) {
		this.__SET_SCALE__({
			x: width,
			y: (width * this.__TEXTURE__.height) / this.__TEXTURE__.width,
		});
	}
	...
	setScaleHeight(height: number) {
		this.__SET_SCALE__({
			x: (height * this.__TEXTURE__.width) / this.__TEXTURE__.height,
			y: height,
		});
	}
}
