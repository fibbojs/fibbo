		...
		for (let j = 0; j < GRID_COLS; j++) {
			const square = new FRectangle({
				position: {
					...
					y: j * GRID_GAP - (GRID_COLS * GRID_GAP) / 2,
				},
				scale: { x: 0.5, y: 0.5 },
				color: 0x2c2c2c,
			});
			...
			square.__CONTAINER__.on("mouseenter", () => {
				...
				const distance = 8;
				for (
					let x = Math.max(0, i - distance);
					x <= Math.min(GRID_ROWS - 1, i + distance);
					x++
				) {
					for (
						let y = Math.max(0, j - distance);
						y <= Math.min(GRID_COLS - 1, j + distance);
						y++
					) {
