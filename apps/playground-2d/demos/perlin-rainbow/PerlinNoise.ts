/**
 * Perlin noise implementation from : https://github.com/DerkJanSpeelman/ts-perlin-noise
 */
export class PerlinNoise {
  ...
  public noise = (x: number, y: number = 0, z: number = 0): number => {
    ...
    const X: number = Math.floor(x) & 255
    const Y: number = Math.floor(y) & 255
    ...
    const A: number = p[X] + Y
    const AA: number = p[A] + Z
    const AB: number = p[A + 1] + Z
    const B: number = p[X + 1] + Y
    const BA: number = p[B] + Z
    const BB: number = p[B + 1] + Z
  ...
  public getGreyScale(x: number, y: number, z: number): number {
    const noiseValue = this.noise(x, y, z)
    const normalizedNoise = (noiseValue * 128) + 128
    return normalizedNoise << 16 | normalizedNoise << 8 | normalizedNoise
  }
}
